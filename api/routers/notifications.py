...
async def list_notifications(
    ...
    status: str
    | None = Query(None, description="Filter by status (pending, sent, delivered, etc.)"),
    type: str
    | None = Query(None, description="Filter by type (assignment, reminder, update, cancellation)"),
...
async def get_notification(
    ...
    **Multi-tenant**: Verified by recipient_id
    ...
    if notification.recipient_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Access denied: You can only view your own notifications"
        )
...
async def get_my_email_preferences(
    ...
    **RBAC**: Authenticated user
    """
    email_pref = (
        db.query(EmailPreference).filter(EmailPreference.person_id == current_user.id).first()
    )
...
async def get_organization_notification_stats(
    ...
    **Multi-tenant**: Filtered by org_id
    ...
    status_counts = (
        db.query(Notification.status, func.count(Notification.id).label("count"))
        .filter(Notification.org_id == org_id, Notification.created_at >= cutoff_date)
...
async def send_test_notification(
    ...
    **Multi-tenant**: Verified by admin's org_id
    ...
    test_notification = create_notification(
        ...
        template_data={"test_mode": True, "recipient_email": recipient_email},
        db=db,
