...
class SubscriptionResponse(BaseModel):
    ...
    billing_cycle: str | None = Field(None, description="Billing cycle (monthly, annual)")
    status: str = Field(
        ..., description="Subscription status (active, trialing, past_due, cancelled)"
    )
    trial_end_date: datetime | None = Field(None, description="Trial end date")
    ...
    pending_downgrade: dict[str, Any] | None = Field(
        None,
        description="Scheduled downgrade details (new_plan_tier, effective_date, credit_amount_cents)",
    )
...
class UpgradeRequest(BaseModel):
    ...
    trial_days: int | None = Field(14, description="Number of trial days (default: 14)")
...
class DowngradeRequest(BaseModel):
    ...
    reason: str | None = Field(None, description="Reason for downgrade")
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"org_id": "org_123", "new_plan_tier": "free", "reason": "Cost reduction"}
        }
    )
...
class ChangePlanRequest(BaseModel):
    ...
    new_billing_cycle: str | None = Field(None, description="New billing cycle (monthly, annual)")
    ...
    @classmethod
    def validate_new_billing_cycle(cls, v: str | None) -> str | None:
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"org_id": "org_123", "new_plan_tier": "pro", "new_billing_cycle": "annual"}
        }
    )
...
class CancelRequest(BaseModel):
    ...
    immediately: bool = Field(
        False, description="Cancel immediately (True) or at period end (False)"
    )
...
class UsageMetricsResponse(BaseModel):
    ...
    plan_limit: int | None = Field(None, description="Plan limit (None = unlimited)")
...
class UsageSummaryResponse(BaseModel):
    ...
    limits: dict[str, int | None] = Field(..., description="Plan limits")
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "usage": {"volunteers_count": {"current": 45, "limit": 50, "percentage": 90.0}},
                "warnings": ["volunteers_count: 45/50 (90% used)"],
            }
        }
    )
...
class BillingHistoryResponse(BaseModel):
    ...
    invoice_pdf_url: str | None = Field(None, description="Invoice PDF URL")
