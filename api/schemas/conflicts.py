...
class ConflictType(BaseModel):
    ...
    type: str = Field(..., description="Conflict type: already_assigned, time_off, double_booked")
    message: str = Field(..., description="Human-readable conflict message")
    conflicting_event_id: str | None = Field(
        None, description="ID of conflicting event if applicable"
    )
    start_time: datetime | None = Field(None, description="Conflict start time")
    end_time: datetime | None = Field(None, description="Conflict end time")
