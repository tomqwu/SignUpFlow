...
async function changeLanguage(locale) {
    ...
    // Update title by re-applying view
    const currentView = localStorage.getItem('roster_current_view') || 'schedule';
...
async function saveSettings() {
    try {
        ...
        if (response.ok) {
            ...
            if (needsReload) {
                translatePage();
                // Also re-apply current view to update title (which doesn't use data-i18n)
                // Update title by re-applying view
                const currentView = localStorage.getItem('roster_current_view') || 'schedule';
                if (typeof switchView === 'function') {
                    switchView(currentView, true);
...
window.switchAdminTab = function(tabName) {
    ...
    switch (tabName) {
        ...
        case 'teams':
            ...
            console.log('typeof loadAdminTeams:', typeof loadAdminTeams);
            if (typeof loadAdminTeams === 'function') {
                loadAdminTeams();
            } else {
                console.error('loadAdminTeams is not a function!');
            }
