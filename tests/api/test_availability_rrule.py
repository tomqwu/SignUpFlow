...
class TestAvailabilityRrule:
    ...
    def test_put_sets_rrule_and_get_returns_it(self, client):
        org_id = "rr-set"
        seed_org(client, org_id)
        person_id, headers = _person_for(client, org_id, "s")
        ...
        get_resp = client.get(f"/api/v1/availability/{person_id}/rrule", headers=headers)
        assert get_resp.json() == {"rrule": "FREQ=WEEKLY;BYDAY=MO"}

    def test_put_replaces_existing_rrule(self, client):
        org_id = "rr-replace"
        seed_org(client, org_id)
        person_id, headers = _person_for(client, org_id, "r")
    ...
    def test_delete_clears_rrule(self, client):
        ...
        del_resp = client.delete(f"/api/v1/availability/{person_id}/rrule", headers=headers)
    ...
    def test_delete_when_never_set_is_idempotent(self, client):
        ...
        resp = client.delete(f"/api/v1/availability/{person_id}/rrule", headers=headers)
        assert resp.status_code == 204

    def test_put_for_unknown_person_returns_404(self, client):
        org_id = "rr-noperson"
        seed_org(client, org_id)
        _, headers = _person_for(client, org_id, "n")
        resp = client.put(
    ...
    def test_put_empty_string_rejected(self, client):
        ...
        seed_org(client, org_id)
        person_id, headers = _person_for(client, org_id, "v")
        resp = client.put(
            f"/api/v1/availability/{person_id}/rrule",
            json={"rrule": ""},
            headers=headers,
