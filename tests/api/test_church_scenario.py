...
class TestChurchScenario:
    ...
    MEMBERS = [
        {"email": "sarah.chen@grace.org",   "name": "Sarah Chen",
         "roles": ["musician", "teacher"]},
        {"email": "david.kim@grace.org",     "name": "David Kim",
         "roles": ["musician", "sound_tech"]},
        {"email": "maria.lopez@grace.org",   "name": "Maria Lopez",
         "roles": ["teacher", "volunteer"]},
        {"email": "james.brown@grace.org",   "name": "James Brown",
         "roles": ["usher", "volunteer"]},
        {"email": "emily.davis@grace.org",   "name": "Emily Davis",
    ...
    def test_church_setup_and_member_roles(self, client):
        ...
        sarah = members["sarah.chen@grace.org"]
    ...
    def test_ministry_teams(self, client):
        ...
        worship = seed_team(client, hdrs, self.ORG, "worship-team",
                           "Worship Team", member_ids=worship_ids)
        ...
        teaching = seed_team(client, hdrs, self.ORG, "teaching-team",
                            "Teaching Team", member_ids=teach_ids)
    ...
    def test_sunday_events_with_roles(self, client):
        ...
        worship = seed_event(client, hdrs, self.ORG, "sunday-worship",
        ...
        school = seed_event(client, hdrs, self.ORG, "sunday-school",
        ...
        youth = seed_event(client, hdrs, self.ORG, "wed-youth",
    ...
    def test_member_vacation(self, client):
        ...
        add_timeoff(client, maria_id, vacation_start, vacation_end,
        ...
        add_timeoff(client, james_id, wed_date, wed_date,
    ...
    def test_solver_schedules_sunday_services(self, client):
        ...
        for week in range(4):
            seed_event(client, hdrs, self.ORG, f"worship-wk{week}",
                      event_type="Sunday Worship",
    ...
    def test_pastor_manually_assigns_worship_leader(self, client):
        ...
        event = seed_event(client, hdrs, self.ORG, "special-sunday",
        ...
        assert resp.status_code == 200
        assignments = resp.json()["assignments"]
        sarah_assigned = [a for a in assignments
    ...
    def test_member_views_own_assignments(self, client):
        ...
        for week in range(2):
            seed_event(client, hdrs, self.ORG, f"svc-{week}",
                      event_type="Sunday Worship",
                      days_from_now=14 + (week * 7),
        ...
        client.post("/api/solver/solve", json={
            "org_id": self.ORG, "from_date": from_date,
            "to_date": to_date, "mode": "relaxed", "change_min": False,
        }, headers=hdrs)
    ...
    def test_new_member_joins_via_invitation(self, client):
        ...
        inv = seed_invitation(client, hdrs, self.ORG,
                             "rachel.wong@gmail.com", "Rachel Wong",
                             roles=["volunteer", "teacher"])

        # Verify invitation is pending
        resp = client.get(f"/api/invitations?org_id={self.ORG}", headers=hdrs)
        assert resp.status_code == 200
        pending = [i for i in resp.json()["invitations"]
                  if i["email"] == "rachel.wong@gmail.com"]
