"""Six-week church and basketball acceptance journeys, without mocked auth."""
...
def test_six_week_playbook(client, domain):
    p = Playbook(client, domain)
    for week in range(6):
        p.event(week)
        p.event(week, "rehearsal" if domain == "church" else "practice", hour=18, day_offset=3)
    ...
    critical = next(iter(p.spec["roles"]))
