...
class TestPeopleAPI:
    ...
    def test_update_person_roles(self):
        ...
        response = requests.patch(f"{API_BASE}/people/person_jane_1759550943", json=data)
        assert response.status_code == 200
...
class TestEventsAPI:
    ...
    def test_get_available_people(self):
        ...
        response = requests.get(f"{API_BASE}/events/event_001/available-people")
        assert response.status_code == 200
    ...
    def test_event_validation(self):
        ...
        response = requests.get(f"{API_BASE}/events/event_001/validation")
        assert response.status_code == 200
        data = response.json()
...
class TestAvailabilityAPI:
    ...
    def test_add_blocked_date(self):
        ...
        response = requests.post(
            f"{API_BASE}/availability/person_jane_1759550943/timeoff",
            json=data
        )
    ...
    def test_update_blocked_date(self):
        ...
        if len(timeoff_list) > 0:
            ...
            response = requests.patch(
                f"{API_BASE}/availability/person_jane_1759550943/timeoff/{timeoff_id}",
                json=data
            )
            assert response.status_code == 200
...
class TestAssignmentsAPI:
    ...
    def test_assign_person_to_event(self):
        ...
        data = {"person_id": "person_sarah_1759550943", "action": "assign"}
        response = requests.post(
            f"{API_BASE}/events/event_001/assignments",
            json=data
        )
        assert response.status_code in [200, 201, 409]  # 409 if already assigned

    def test_unassign_person_from_event(self):
        """Unassign a person from an event"""
        data = {"person_id": "person_sarah_1759550943", "action": "unassign"}
        response = requests.post(
            f"{API_BASE}/events/event_001/assignments",
            json=data
        )
...
class TestPDFExportAPI:
    ...
    def test_pdf_export_has_blocked_markers(self):
        ...
        # First, ensure Jane has a blocked date on Oct 11
        data = {
...
class TestGUIEventManagement:
    ...
    def test_event_list_shows_blocked_warnings(self):
...
class TestGUIAssignmentModal:
    ...
    def test_assignment_modal_shows_blocked_badge(self):
...
class TestGUIBlockedDates:
    ...
    def test_add_blocked_date_with_reason(self):
...
class TestBlockedDatesIntegration:
    ...
    def test_blocked_person_shown_in_validation(self):
        ...
        # Ensure Jane has blocked date on Oct 11
        data = {
            "start_date": "2025-10-11",
            "end_date": "2025-10-11",
